$(document)
  .ready(() => {
    ...
    $(function () {
      ...
      if (!localStorage[tabid].startingDate) {
        localStorage[tabid].startingDate = new Date()
      }
      let startingDate = localStorage[tabid].startingDate

      console.log(localStorage[tabid].startingDate);
      (function timerTick() {
        setTimeout(function () {
          $timer.text(secondsFormat(
                        (Math.abs(startingDate.getTime() - (new Date()).getTime()) / 1000).toFixed(0)))
          timerTick()
        }, 1000);
      })()
